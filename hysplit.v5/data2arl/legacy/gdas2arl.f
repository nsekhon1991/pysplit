...
!
! MAIN PROGRAM: GDAS2ARL     DECODE GRIB GFS MODEL FIELD FOR HYSPLIT
...
!                 02 Aug 2004 (BS)  - add fields for GDAS archive (1 degree)
!                 31 May 2005 (BS)  - change SOLM from 10-200 cm to 10-40 cm
!                                       NCEP divide 10-200 cm into 3 layers
!                 10 Nov 2005 (BS)  - add '+N2DV' to NREC=1+N2DV before
!                                           do CALL ROTATE, doesn't affect -g3
!                                           lat-lon grid
!                 03 Mar 2008 (BS)  - change DATA.GFS DATA.GDAS
!                 01 Dec 2009 (BS)  - precip and fluxes at f00
...
! output array limits
  ...
  INTEGER, PARAMETER   :: N2DV=29     ! number of 2D variables 
...
!  grid id (kpds5)
  DATA VGRIB0/  1,     2,    61,   124,   125,   122,   204,    52, &
              ...
              132,   221,    11,    63,   144,   143,   142, &
              141,   140,   121,    71,    71,    71/
!  level id (kpds7)
  DATA SIG0/    0,     0,     0,     0,     0,     0,     0,     2, &
               10,    10,     2,     0,     0,     0,     0,     0, &
                0,     0,     0,     0,  2600,     0,     0, &
                0,     0,     0,     0,     0,     0/ 
     ! SOLM sig0=2760 and sig0=10                 SOLM 10-200 cm
     ! SOLM sig0=2600 and sig0=10 ! May 31, 2005, SOLM 10-40 cm
!  type (kpds6)
...
!  arl id
  DATA VCHAR0  &
          /'PRSS','MSLP','TPP6','UMOF','VMOF','SHTF','DSWF','RH2M', &
           ...
           'LIB4','PBLH','TMPS','CPP6','SOLM','CSNO','CICE', &
           'CFZR','CRAI','LHTF','LCLD','MCLD','HCLD'/
...
! standard output name for packed data
  INQUIRE(FILE='DATA.GDAS',EXIST=FTEST)
  LREC=NXY+50
  OPEN(20,FILE='DATA.GDAS',RECL=LREC,ACCESS='DIRECT',FORM='UNFORMATTED')
...
! go to next grib record if not one degree lat-lon grid (3)
  IF(IGRID.NE.3) THEN
    WRITE(*,*)'skip record - Grid is not one degree lat-lon',IGRID
    GO TO 800
  END IF
...
!      matches id, special level indicator, level type, and time range
       IF(KVARB.EQ.VGRIB.AND.         &
          ...
          ! if field is a bitmap, skip it (ok to have GDS without bitmap, value 128)
            ...
            ! soil moisture is a bitmap but since using ncep w3fi63, not hysplit,  
            !   and because of 1,1 value and range of values it is OK, do not skip
              IF(KVARB.NE.144)THEN
...
200 DO KL=1,NLVL
       ...
       !  RELH at 20 mb
          IF(KVARB.EQ.VGRIB1(5).AND.(LEVEL.EQ.SIGL(22).OR.LEVEL.EQ.SIGL(23)))GO TO 800
...
!                 10 Jul 2002 (BJS) - add fields, levels
!                 02 Aug 2004 (BJS) - increase possible number fields per level
!
