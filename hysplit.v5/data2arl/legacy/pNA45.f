...
!
! PROGRAM:     PNA45         DECODE GRIB MM5 MODEL FIELD FOR HYSPLIT
...
!
! PROGRAM HISTORY LOG:
!     26 Jan 01 (GRD) - original version
!     10 Sep 02 (RRD) - upgrade to match revised w3lib
!     18 Aug 03 (GDR) - corrected TCLD field ID number
!
! USAGE: PNA45 [GRIB_FILE_NAME] [GRIB_FILE_NAME2] [...]
!   INPUT ARGUMENT LIST:
...
!     check for command line arguments
      narg=IARGC()
      if(narg.eq.0)then
         write(*,*)'Usage: pNA45 [file_1] [file_2] [...]'
         write(*,*)'Convert 45 km pressure mm5 grib file to arl format'
         write(*,*)'File: us057g1010t02u${cyc}000${fhr}00.gz'
         stop
      end if
...
!     number of levels, variables, and array limits
      PARAMETER (NLVL=25, MVAR=17, MAXX=192, MAXY=139, MAXB=100000)
...
!     set output structure in ARL character format
      DATA VCHAR0/ 'MSLP','T02M','TPP3','RH2M','U10M','V10M','PRSS',           &
...
!    special case for initialization hour of MM5??????
               IF(KTIM1.EQ.0.AND.KTIM3.EQ.1)GO TO 300
            ELSE
               KTIMD=KTIM2-KTIM1
...
!$$$
      ...
      SUBROUTINE DATINI(KUNIT,CVAR,NXY,NLVL,NVAR,IG,IY,IM,ID,IH,VCHAR0,        &
                        MVAR)
...
!     index record
      ...
      DO NV=1,NVAR(NL)
       ...
       IL=NL-1
!   write the 4 letter ID because the first time period some/all surface
