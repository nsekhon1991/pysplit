...
!
!  PROGRAM:     KMA2ARL         DECODE KMA GRIB ETA MODEL FOR HYSPLIT
!   PRGMMR:    GLENN ROLPH      ORG: R/ARL       DATE:99-01-29
!
! ABSTRACT:  THIS CODE WRITTEN AT THE AIR RESOURCES LABORATORY ...
!     converts an mm5 model pressure grib file to arl packed format
...
!
! PROGRAM HISTORY LOG:
!   LAST REVISED: 07 Jul 2004 (RRD) - original version from afwa2arl
!                 23 Jul 2004 (RRD) - 30 km data with vertical velocity
!
! USAGE: KMA2ARL [GRIB_FILE_NAME] [GRIB_FILE_NAME2] [...]
!   INPUT ARGUMENT LIST:
...
!     check for command line arguments
      narg=IARGC()
      if(narg.eq.0)then
         write(*,*)'Korean Meteorological Agency pressure grib file to arl format'
         write(*,*)'Usage: kma2arl [file_1] [file_2] [...]'
         stop
      end if
...
!     number of levels, variables, and array limits
      PARAMETER (NLVL=25, MVAR=6, MAXX=200, MAXY=200, MAXB=200000)
...
!     special surface variables level identification
      ...
      DATA STYP/  1,   1,   1,   1, 102,   0/ 

!     set output structure by defining GRIB variables (sfc + level 1)
      DATA VGRIB0/  7, 11, 33, 34,  2,  0/
      DATA VGRIB1/ 33, 34, 40,  7, 11, 53/

!     set output structure in ARL character format
      DATA VCHAR0/ 'SHGT','T02M','U10M','V10M','MSLP','    '/
      DATA VCHAR1/ 'UWND','VWND','WWND','HGTS','TEMP','SPHU'/
...
!     units conversion factors before converting to ARL format
      DATA CNVRT0/  1.0, 1.0, 1.0, 1.0, 0.01, 0.0/
...
!     product definition section (+8 byte indicator section offset)
      koff=8
      kvarb=ichar(buff(koff+9))
      ktype=ichar(buff(koff+10))
      level=ichar(buff(koff+12))+ichar(buff(koff+11))*256
!     check the hours at octet 19 and 20 for precipitation summing
      ktim1=ichar(buff(koff+19))
      ktim2=ichar(buff(koff+20))
!     check the octect 21 to remove all averaged fields
      ktim3=ichar(buff(koff+21))
...
!     perform any required units conversion
      if(cnvrt.ne.1.0) then
	     call datcnv(rvar,nxp,nyp,cnvrt)

      elseif(cnvrt.eq.-1.0)then
!        vertical velocity conversion from m/s to hPa/s is hardwired
!        with this version because temperature and pressure fields are
!        not consecutive within the file. A constant temperature assumed.
!
!        dz   Temp * Rdry dP    273 * 287.04  dP    8000 dP
!        -- = ----------- -- =  ------------  -- =  ---- --
!        dt   Grav * Pres dt    9.8 * Pres    dt    Pres dt

         factor=cnvrt*level/8000.0
	     call datcnv(rvar,nxp,nyp,factor)
      end if
