/*
 * 
 * $Source: /home/greatdev/src/analysis/txt2dbf/lib/RCS/dbf.c,v $
 * 
 * $Author: fkoorman $
 * 
 * $Revision: 1.1 $
 ...
 *	lines 138, 171, 409:	prepare data area with memset
 *	lines 128f, 144f   :	get system time and set file date
 *	line 429ff         :	empty contents results empty fields
 *	line 433	   :	set formatstring for real numbers
 */
...
#include "dbf.h"
/* The contents of endian.h seems to be included in dbf.h. Following line is not
necessary: 
#include "endian.h"
*/
...
/* read in the disk-header */
	...
	dbh->db_nfields = (dbh->db_hlen - sizeof(dbf_header)) 
				/ sizeof(dbf_field);
...
/* NEW: get systemtime and set filedate to it: */
...
/* Correction 2018/06/20 Albion Taylor sizeof(dbhead) replaced by sizeof(dbf_header) */
	dbh->db_records = 0;
	dbh->db_currec = 0;
	dbh->db_rlen = 1;
	dbh->db_nfields = 0;
...
field *dbf_build_record(dbhead *dbh) {
	...
	for ( t = 0; t < dbh->db_nfields; t++) {
		...
		strncpy(fields[t].db_name, dbh->db_fields[t].db_name, 
							DBF_NAMELEN);
		fields[t].db_type = dbh->db_fields[t].db_type;
		fields[t].db_flen = dbh->db_fields[t].db_flen;
		fields[t].db_dec  = dbh->db_fields[t].db_dec;
...
/*MODIFIED: if field is empty, only copy... */	
				if (strcmp(rec[t].db_contents,"") == 0)
					strcpy(foo, rec[t].db_contents);
				else
				{	fl = atof(rec[t].db_contents);
/* MODIFIED:				sprintf(format, "%%.%d", rec[t].db_dec);
*/
