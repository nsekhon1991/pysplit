...
proc HMinit_win {win} {
	...
	$win tag bind link <1> [namespace code "HMlink_hit $win %x %y" ]
}
...
#   $win tag configure hr -borderwidth $width
...
proc HMgoto {win where {callback HMwent_to}} {
	...
	if {[regexp N:$where [$win mark names]]} {
      for {set temp [$win cget -height]} {$temp > 0} {incr temp -5} {
        $win see [expr [$win index N:$where] + $temp.0]
      }
      $win see N:$where
		update
